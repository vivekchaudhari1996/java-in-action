...
public class ArticulationPointsDFS {
    ...
    // Solution:
    // 1. Take three arrays vis, tin, low.
    // 2. Take articulation array for output.
    // 3. Perform dfs.
    // 4. Two conditions:
    // 5. low[it] >= tin[node] && parent != -1
    // 6. parent == -1 && child > 1
    ...
    private static void dfs(int node, int parent, int vis[], int tin[], int low[], ArrayList<ArrayList<Integer>> adj, int timer, int isArticulation[]) {
        vis[node] = 1;
