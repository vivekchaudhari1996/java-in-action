...
public class BinaryIndexedTreeOrFenwick {
...
arr[0..n-1] --> Input array for which prefix sum
    ...
    // Finding lower bound till where sum of array is >=v
    // Binary Lifting
    int findLowerBound(int v, int n){
        int curr=0;
        int prevSum = 0;
        int maxI = (int)(Math.log(n) / Math.log(2));
        for(int i = maxI;i>=0;i--){
            if(BITree[curr + (1<<i)] + prevSum < v){
                curr = curr + 1<<i;
                prevSum = prevSum + BITree[curr];
            }
        }
        return curr+1;
    }
