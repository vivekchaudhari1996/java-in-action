...
public class MultiSetAndQueriesBinaryLifting {
    ...
    // Now perform -ve query: Use Binary Lifting to find which nbr to delete.
    ...
    {
        int freq[] = {2, 1, 1, 3, 2, 3};  // Assuming nbrs are between 1->6
        ...
        for(int i=0;i<queries.length;i++){
            ...
            if(x > 0){
                ...
                x = Math.abs(x);
                int index = tree.findLowerBound(x, N+1);
                tree.updateBIT(N+1, index, -1); // logN
            }
        }
