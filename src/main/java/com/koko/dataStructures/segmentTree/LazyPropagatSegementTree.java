...
public class LazyPropagatSegementTree {

    // Q:
    // What if there are updates on a range of array indexes?
    // For example add 10 to all values at indexes from 2 to 7 in array.
    // The update has to be called for every index from 2 to 7.
    // We can avoid multiple calls by writing a function updateRange() that updates nodes accordingly.
    ...
    // Initialize all elements of lazy[] as 0. A value 0 in lazy[i] indicates that there are no pending updates on node i
    // in segment tree. A non-zero value of lazy[i] means that this amount needs to be added to node i in segment tree
...
...b) Update current node using results of left
    ...
    public static void main(String args[])
    {
        ...
        int n = arr.length;
        LazyPropagatSegementTree tree = new LazyPropagatSegementTree();
